...
public class TraceCommandHandler implements CommandHandler {
  ...
  public void handle(Args args, CommandDispatcher dispatcher) throws Exception {
    ...
    MethodInfo methodInfo = CommandHelper.findMethod(args.arg("trace-expression"));
    if (methodInfo != null) {
      configStrategy(args, methodInfo);
      // configStrategy2(args);
      trace(methodInfo);
      // trace2();
      // 不结束，等待消息
  ...
  private void trace(MethodInfo methodInfo) {
    ClassLoaderHelper.resetClassLoader(methodInfo.getMethod().getDeclaringClass());
  ...
  private void configStrategy(Args args, MethodInfo methodInfo) throws Exception {
    int traceNum = 1;
    ...
    if (fOptions != null) {
      filters.add(new PatternMethodFilter(fOptions.get("filter-expression")));
    }
    filters.add(new ProxyMethodFilter());
    filters.add(new TraceMethodFilter(methodInfo.getThisObject(), methodInfo.getClazz(),
  ...
  public void declareArgs(ArgDef argDef) {
    ...
    argDef.setDefinition(
        "trace [-f <filter-expression>] [-n <trace-num>]  [-c] [-t] <trace-expression>");
