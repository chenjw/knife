...
import com.chenjw.knife.bytecode.javassist.Expression;
import com.chenjw.knife.bytecode.javassist.Field;
...
public final class NewMethodGenerator implements MethodGenerator {
	...
	private NewMethodGenerator(String modifier, String name,
		...
		sb.append(modifier);
		sb.append(" ");
		sb.append(ClassHelper.makeClassName(returnType));
		sb.append(" ");
		sb.append(name);
		sb.append("(");
		if (paramTypes != null && paramTypes.length > 0) {
			...
			for (Class<?> paramType : paramTypes) {
				...
				sb.append(ClassHelper.makeClassName(paramType));
			}
		}
		sb.append(")");
		if (exceptionTypes != null && exceptionTypes.length > 0) {
			...
			for (Class<? extends Throwable> exceptionType : exceptionTypes) {
				...
				sb.append(ClassHelper.makeClassName(exceptionType));
			}
		}
	...
	public Field addLocalVariableDefine(Class<?> type, Expression expr) {
		...
		sb.append(ClassHelper.makeClassName(type)).append(' ');
		sb.append(name);
		if (expr != null) {
			sb.append('=');
