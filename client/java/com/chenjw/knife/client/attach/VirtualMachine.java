...
public class VirtualMachine {
	...
	public VirtualMachine(AttachProvider provider, String id) {
		if (provider == null) {
	...
	public VirtualMachine attach(VirtualMachineDescriptor vmd) {
	...
	public void loadAgentPath(String agentPath) throws AgentLoadException,
			AgentInitializationException, IOException {
		loadAgentPath(agentPath, null);
	}

	public void loadAgent(String agent) throws AgentLoadException,
			AgentInitializationException, IOException {
		loadAgent(agent, null);
	}

	/*
	 * Load agent library If isAbsolute is true then the agent library is the
	 * absolute path to the library and thus will not be expanded in the target
	 * VM. if isAbsolute is false then the agent library is just a library name
	 * and it will be expended in the target VM.
	 */
	private void loadAgentLibrary(String agentLibrary, boolean isAbsolute,
			String options) throws AgentLoadException,
			AgentInitializationException, IOException {
		InputStream in = execute("load", agentLibrary, isAbsolute ? "true"
				: "false", options);
		try {
	...
	/*
	 * Load JPLIS agent which will load the agent JAR file and invoke the
	 * agentmain method.
	 */
	public void loadAgent(String agent, String options)
		...
		try {
			 ...
			 * (FIXME: create a better interface to the instrument
	...
	InputStream execute(String cmd, Object... args) throws IOException {
		return this.provider.execute(cmd, args);
	}

	/*
	 * Convenience method for simple commands
	 */
	private InputStream executeCommand(String cmd, Object... args)
			throws IOException {
		try {
	...
	/*
	 * Return attach timeout based on the value of the
	 * sun.tools.attach.attachTimeout property, or the default timeout if the
	 * property is not set to a positive value.
	 */
	long attachTimeout() {
		if (attachTimeout == 0) {
			synchronized (this) {
				if (attachTimeout == 0) {
					try {
						String s = System
								.getProperty("sun.tools.attach.attachTimeout");
