...
public abstract class CommandConsoleTemplate extends CommandListenerTemplate {
  ...
  private FormaterManager formaterManager = new FormaterManager(new ClientPrinter(), this,
  ...
  private List<VMDescriptor> sendListVm() {
    ...
    for (int i = 0; i < vmList.size(); i++) {
      handleText(i + ". " + vmList.get(i).getPid() + " " + vmList.get(i).getName());
    }
  ...
  private boolean sendRequestAttachVm(List<VMDescriptor> vmList) {
    ...
    while (true) {
      ...
      if (0 <= n && n < vmList.size()) {
        break;
      } else {
        boolean found = false;
        for (int i = 0; i < vmList.size(); i++) {
          VMDescriptor vm = vmList.get(i);
          if (StringHelper.equals(vm.getPid(), String.valueOf(n))) {
            n = i;
            found = true;
            break;
          }
