...
import com.chenjw.knife.client.attach.provider.RemoteAttachProvider;
...
public class NewClient {
	...
	private AttachProvider provider = null;
	private volatile boolean isClosed = true;

	public NewClient(int port) {
		this.port = port;
	}
	...
	private void attachAgent(String agentPath) throws NumberFormatException,
			IOException {
		provider = new RemoteAttachProvider("127.0.0.1", 1099);
		List<VirtualMachineDescriptor> list = provider.listVirtualMachines();
	...
	private void attachAgent(String pid, String agentPath) throws IOException {
		try {
			vm = new VirtualMachine(provider, pid);
