...
public final class ClientMain {
	...
	private static void install(String ip, String userName, String password,
			String programePath) {
		try {
			boolean neadReinstall = true;
			System.out.print("local version...");
			String localVersion = FileHelper.readFileToString(new File(
					programePath + "/VERSION"), "UTF-8");
			System.out.println(localVersion);
			System.out.print("remote version...");
			byte[] versionFile = InetHelper.scpGet(ip, userName, password,
					"/tmp/knife/VERSION");
			if (versionFile != null) {
			...
			if (neadReinstall) {
				...
				InetHelper.scpPut(ip, userName, password, programePath,
						"/tmp/", "knife");
