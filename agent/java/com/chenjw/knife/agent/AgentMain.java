...
public class AgentMain {
  ...
  public static void agentmain(String arguments, Instrumentation inst) throws Exception {
    ...
    try {
      ...
      AgentClassLoader
          .setAgentClassLoader(new AgentClassLoader(urls, ClassLoader.getSystemClassLoader()));
      // Thread.currentThread().setContextClassLoader(classLoader);
      Class<?> classAgentServer =
          AgentClassLoader.getAgentClassLoader().loadClass("com.chenjw.knife.agent.AgentServer");
      Constructor<?> constructor =
          classAgentServer.getConstructor(new Class<?>[] {int.class, Instrumentation.class});
      Thread thread =
          new Thread((Runnable) constructor.newInstance(Integer.parseInt(args.get("port")), inst),
              "agent-server");
